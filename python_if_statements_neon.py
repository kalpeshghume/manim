"""
Python if Statements — Neon Light Theme, single scene
...
Structure:
    Intro           — what an if statement is for, preview of the 4 variations
    1. if
    2. if...else
    3. if...elif...else
    4. Nested if
...
Each of the 4 statement topics has TWO beats under the same subtitle:
  (a) a flowchart is drawn, then the path actually taken by that
      topic's example is traced/highlighted in green, with a one-line
      result caption — this is the "show the flow chart" part.
  (b) the flowchart fades and a code panel (IDE-style gutter + real
      indentation) appears on the left with a neon terminal on the
      right that fills in with real print() output as each line that
      ACTUALLY executes gets highlighted; any branch that was skipped
      for that example is dimmed at the end so it's visually clear it
      never ran.

Screen clears gracefully between every top-level topic. Single Scene
subclass on purpose — some Manim runners (including MCP-based online
renderers) only support one renderable scene per file.

NOTE ON SOURCES: topic order (if / if-else / if-elif-else / nested if)
follows programiz.com's "Python if, if...else Statement" tutorial, but
every variable name, value, and flowchart example is original — not the
article's number>0 "positive number" checks, its grade-A/B/C intro
example, or its number>=0/number==0 nested-if example. Examples here
use speed/age/temperature/shipping scenarios with different values
(82, 15, 3, member+weight=4) instead.
"""
...
class IfStatementsScene(Scene):
    ...
    def section_intro(self):
        ...
        self.play(LaggedStart(*[GrowFromCenter(c) for c in chips], lag_ratio=0.15), run_time=1.3)
        self.track(chips)
        self.wait(1.2)
    ...
    def section_if(self):
        ...
        self.play(Create(flowchart), run_time=1.5)
        ...
        self.run_code_beat(desc, code, outputs, exec_order=[0, 1, 2, 3], skip=[], filename="if_demo.py")
    ...
    def section_if_else(self):
        ...
        self.run_code_beat(desc, code, outputs, exec_order=[0, 1, 3, 4], skip=[2], filename="if_else_demo.py")
    ...
    def section_elif(self):
        ...
        self.play(Create(flowchart), run_time=1.7)
        self.track(flowchart)
        ...
        self.run_code_beat(desc, code, outputs, exec_order=[0, 1, 3, 5, 6], skip=[2, 4], filename="elif_demo.py")
    ...
    def section_nested(self):
        ...
        self.run_code_beat(
            desc, code, outputs,
            exec_order=[0, 1, 2, 3, 4], skip=[5, 6, 7, 8], filename="nested_if_demo.py",
        )
    ...
    def run_code_beat(self, desc, code, outputs, exec_order, skip, filename):
