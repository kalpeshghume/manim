...
class IfStatementsScene(VoiceoverScene):
    ...
    def build_if_flowchart(self):
        ...
        arrow_start_decision = self.flow_arrow(start.get_bottom(), decision.get_top(), NEON_CYAN)
        arrow_decision_yes = self.flow_arrow(decision.get_left(), yes_box.get_top(), NEON_GREEN, "Yes", LEFT * 0.35)
        arrow_decision_no = self.flow_arrow(decision.get_right(), done_box.get_right(), NEON_MAGENTA, "No", RIGHT * 0.35 + UP * 0.2)
        arrow_yes_done = self.flow_arrow(yes_box.get_bottom(), done_box.get_top(), NEON_CYAN)
        arrow_done_end = self.flow_arrow(done_box.get_bottom(), end.get_top(), NEON_CYAN)

        arrows = VGroup(arrow_start_decision, arrow_decision_yes, arrow_decision_no, arrow_yes_done, arrow_done_end)
        group = VGroup(start, decision, yes_box, done_box, end, arrows)
        self.clamp_flowchart(group)

        # Path taken: start -> decision -> yes -> done -> end
        path_nodes = [start, decision, yes_box, done_box, end]
        path_arrows = [arrow_start_decision, arrow_decision_yes, arrow_yes_done, arrow_done_end]

        return {
            "group": group,
            "nodes": {"start": start, "decision": decision, "yes_box": yes_box,
                      "done_box": done_box, "end": end},
            "arrows": {
                "start_decision": arrow_start_decision,
                "decision_yes": arrow_decision_yes,
                "decision_no": arrow_decision_no,
                "yes_done": arrow_yes_done,
                "done_end": arrow_done_end,
            },
            "path_nodes": path_nodes,
            "path_arrows": path_arrows,
        }
    ...
    def build_ifelse_flowchart(self):
        ...
        arrow_start_decision = self.flow_arrow(start.get_bottom(), decision.get_top(), NEON_CYAN)
        arrow_decision_yes = self.flow_arrow(decision.get_left(), yes_box.get_top(), NEON_GREEN, "Yes", LEFT * 0.35)
        arrow_decision_no = self.flow_arrow(decision.get_right(), no_box.get_top(), NEON_MAGENTA, "No", RIGHT * 0.35)
        arrow_yes_end = self.flow_arrow(yes_box.get_bottom(), end.get_left() + UP * 0.15, NEON_CYAN)
        arrow_no_end = self.flow_arrow(no_box.get_bottom(), end.get_right() + UP * 0.15, NEON_CYAN)

        arrows = VGroup(arrow_start_decision, arrow_decision_yes, arrow_decision_no, arrow_yes_end, arrow_no_end)
        group = VGroup(start, decision, yes_box, no_box, end, arrows)
        self.clamp_flowchart(group)

        # Path taken: start -> decision -> no -> end (since age=15)
        path_nodes = [start, decision, no_box, end]
        path_arrows = [arrow_start_decision, arrow_decision_no, arrow_no_end]

        return {
            "group": group,
            "nodes": {"start": start, "decision": decision, "yes_box": yes_box, "no_box": no_box, "end": end},
            "arrows": {
                "start_decision": arrow_start_decision,
                "decision_yes": arrow_decision_yes,
                "decision_no": arrow_decision_no,
                "yes_end": arrow_yes_end,
                "no_end": arrow_no_end,
            },
            "path_nodes": path_nodes,
            "path_arrows": path_arrows,
    ...
    def build_elif_flowchart(self):
        ...
        arrow_start_d1 = self.flow_arrow(start.get_bottom(), d1.get_top(), NEON_CYAN)
        arrow_d1_yes = self.flow_arrow(d1.get_left(), hot_box.get_right(), NEON_GREEN, "Yes", UP * 0.28)
        arrow_d1_no = self.flow_arrow(d1.get_bottom(), d2.get_top(), NEON_MAGENTA, "No", RIGHT * 0.4)
        arrow_d2_yes = self.flow_arrow(d2.get_left(), mild_box.get_right(), NEON_GREEN, "Yes", UP * 0.28)
        arrow_d2_no = self.flow_arrow(d2.get_bottom(), cold_box.get_top(), NEON_MAGENTA, "No", RIGHT * 0.4)
        arrow_hot_end = self.flow_arrow(hot_box.get_bottom(), end.get_left() + UP * 0.15, NEON_CYAN)
        arrow_mild_end = self.flow_arrow(mild_box.get_bottom(), end.get_left() + UP * 0.05, NEON_CYAN)
        arrow_cold_end = self.flow_arrow(cold_box.get_bottom(), end.get_top(), NEON_CYAN)

        arrows = VGroup(arrow_start_d1, arrow_d1_yes, arrow_d1_no, arrow_d2_yes, arrow_d2_no,
                        arrow_hot_end, arrow_mild_end, arrow_cold_end)
        group = VGroup(start, d1, hot_box, d2, mild_box, cold_box, end, arrows)
        self.clamp_flowchart(group)

        # Path taken: start -> d1 -> d2 -> cold -> end (temp=3)
        path_nodes = [start, d1, d2, cold_box, end]
        path_arrows = [arrow_start_d1, arrow_d1_no, arrow_d2_no, arrow_cold_end]

        return {
            "group": group,
            "nodes": {"start": start, "d1": d1, "hot": hot_box, "d2": d2, "mild": mild_box, "cold": cold_box, "end": end},
            "arrows": {
                "start_d1": arrow_start_d1,
                "d1_yes": arrow_d1_yes,
                "d1_no": arrow_d1_no,
                "d2_yes": arrow_d2_yes,
                "d2_no": arrow_d2_no,
                "hot_end": arrow_hot_end,
                "mild_end": arrow_mild_end,
                "cold_end": arrow_cold_end,
            },
            "path_nodes": path_nodes,
            "path_arrows": path_arrows,
        }
    ...
    def build_nested_flowchart(self):
        ...
        arrow_start_d1 = self.flow_arrow(start.get_bottom(), d1.get_top(), NEON_CYAN)
        arrow_d1_yes = self.flow_arrow(d1.get_left(), d2.get_top(), NEON_GREEN, "Yes", LEFT * 0.35)
        arrow_d1_no = self.flow_arrow(d1.get_right(), standard_box.get_top(), NEON_MAGENTA, "No", RIGHT * 0.35)
        arrow_d2_yes = self.flow_arrow(d2.get_left(), free_box.get_top(), NEON_GREEN, "Yes", LEFT * 0.35)
        arrow_d2_no = self.flow_arrow(d2.get_right(), discount_box.get_top(), NEON_MAGENTA, "No", RIGHT * 0.35)
        arrow_free_end = self.flow_arrow(free_box.get_bottom(), end.get_left() + UP * 0.15, NEON_CYAN)
        arrow_discount_end = self.flow_arrow(discount_box.get_bottom(), end.get_bottom() + DOWN * 0.15, NEON_CYAN)
        arrow_standard_end = self.flow_arrow(standard_box.get_bottom(), end.get_right() + UP * 0.15, NEON_CYAN)

        arrows = VGroup(arrow_start_d1, arrow_d1_yes, arrow_d1_no, arrow_d2_yes, arrow_d2_no,
                        arrow_free_end, arrow_discount_end, arrow_standard_end)
        group = VGroup(start, d1, d2, standard_box, free_box, discount_box, end, arrows)
        self.clamp_flowchart(group)

        # Path taken: start -> d1 (yes) -> d2 (yes) -> free -> end
        path_nodes = [start, d1, d2, free_box, end]
        path_arrows = [arrow_start_d1, arrow_d1_yes, arrow_d2_yes, arrow_free_end]

        return {
            "group": group,
            "nodes": {"start": start, "d1": d1, "d2": d2,
                      "standard": standard_box, "free": free_box,
                      "discount": discount_box, "end": end},
            "arrows": {
                "start_d1": arrow_start_d1,
                "d1_yes": arrow_d1_yes,
                "d1_no": arrow_d1_no,
                "d2_yes": arrow_d2_yes,
                "d2_no": arrow_d2_no,
                "free_end": arrow_free_end,
                "discount_end": arrow_discount_end,
                "standard_end": arrow_standard_end,
            },
            "path_nodes": path_nodes,
            "path_arrows": path_arrows,
        }
    ...
    # --------------------------------------------------- helpers for blink --
    def blink_arrow(self, arrow, n=3, color=NEON_GREEN):
        """Blink an arrow n times with a quick color flash."""
        for _ in range(n):
            self.play(
                arrow.animate.set_color(color),
                run_time=0.15,
    ...
    def highlight_node(self, node):
        self.play(Indicate(node, color=NEON_GREEN, scale_factor=1.08), run_time=0.3)
    ...
    def section_if(self):
        ...
        steps = [
            {
                "text": "We start at the Start node.",
                "add_nodes": [nodes["start"]],
                "add_arrows": [],
                "highlight_node": nodes["start"],
                "blink_arrows": [],
            },
            {
                "text": "Then we check the condition: is speed greater than 80?",
                "add_nodes": [nodes["decision"]],
                "add_arrows": [arrows["start_decision"]],
                "highlight_node": nodes["decision"],
                "blink_arrows": [arrows["start_decision"]],
            },
            {
                "text": "Since speed is 82, the condition is true, so we take the Yes arrow.",
                "add_nodes": [],
                "add_arrows": [arrows["decision_yes"]],
                "highlight_node": None,
                "blink_arrows": [arrows["decision_yes"]],
            },
            {
                "text": "We print the warning inside the process box.",
                "add_nodes": [nodes["yes_box"]],
                "add_arrows": [],
                "highlight_node": nodes["yes_box"],
                "blink_arrows": [],
            },
            {
                "text": "Then we continue to the done box and finally to the end.",
                "add_nodes": [nodes["done_box"], nodes["end"]],
                "add_arrows": [arrows["yes_done"], arrows["done_end"]],
                "highlight_node": nodes["done_box"],
                "blink_arrows": [arrows["yes_done"], arrows["done_end"]],
            },
        ]

        for step in steps:
            with self.voiceover(text=step["text"]) as tracker:
                if step["add_nodes"]:
                    self.play(*[Create(n) for n in step["add_nodes"]], run_time=0.2)
                if step["add_arrows"]:
                    self.play(*[Create(a) for a in step["add_arrows"]], run_time=0.2)
                if step["highlight_node"]:
                    self.highlight_node(step["highlight_node"])
                for arrow in step["blink_arrows"]:
                    self.blink_arrow(arrow, n=2)
                self.wait(0.1)

        with self.voiceover(text="The No path is not taken in this example, so it remains dimmed.") as tracker:
            no_arrow = arrows["decision_no"]
        ...
        line_voice_texts = [
            ...
            "Since it's true, we print the warning.",
            "Then we print the final message."
        ]
        self.run_code_beat(
            desc, code, outputs, exec_order=[0, 1, 2, 3], skip=[],
            filename="if_demo.py",
            line_voice_texts=line_voice_texts
        )
    ...
    def section_if_else(self):
        ...
        steps = [
            ...
            {
                "text": "We check if age is greater than or equal to 18.",
                "add_nodes": [nodes["decision"]],
                "add_arrows": [arrows["start_decision"]],
                "highlight_node": nodes["decision"],
                "blink_arrows": [arrows["start_decision"]],
            ...
            {
                "text": "For our example, age is 15, so the condition is false. We take the No arrow.",
                "add_nodes": [],
                "add_arrows": [arrows["decision_no"]],
                "highlight_node": None,
                "blink_arrows": [arrows["decision_no"]],
            },
            {
                "text": "We go to the 'Not yet eligible' box.",
                "add_nodes": [nodes["no_box"]],
                "add_arrows": [],
                "highlight_node": nodes["no_box"],
                "blink_arrows": [],
            },
            {
                "text": "Finally we reach the end.",
                "add_nodes": [nodes["end"]],
                "add_arrows": [arrows["no_end"]],
                "highlight_node": nodes["end"],
                "blink_arrows": [arrows["no_end"]],
            },
        ]
        ...
        with self.voiceover(text="The Yes path is not taken because age is not 18 or older.") as tracker:
            yes_arrow = arrows["decision_yes"]
            yes_box = nodes["yes_box"]
            yes_end = arrows["yes_end"]
            self.play(FadeIn(yes_arrow, scale=0.9), FadeIn(yes_box, scale=0.9), run_time=0.3)
            self.play(yes_arrow.animate.set_opacity(0.3), yes_box.animate.set_opacity(0.3), run_time=0.3)
        ...
        line_voice_texts = [
            "We set age to 15.",
            "The if condition checks if age is at least 18.",
            "This line is skipped because the condition is false.",
            "We go to the else block.",
            "We print 'Not yet eligible'."
        ]
        self.run_code_beat(
    ...
    def section_elif(self):
        ...
        steps = [
            ...
            {
                "text": "First decision: is temperature greater than 30?",
                "add_nodes": [nodes["d1"]],
                "add_arrows": [arrows["start_d1"]],
                "highlight_node": nodes["d1"],
                "blink_arrows": [arrows["start_d1"]],
            },
            {
                "text": "Temperature is 3, so condition is false. We take the No arrow to the second decision.",
                "add_nodes": [],
                "add_arrows": [arrows["d1_no"]],
                "highlight_node": None,
                "blink_arrows": [arrows["d1_no"]],
            },
            {
                "text": "Second decision: is temperature greater than 15? Also false.",
                "add_nodes": [nodes["d2"]],
                "add_arrows": [],
                "highlight_node": nodes["d2"],
                "blink_arrows": [],
            },
            {
                "text": "Again false, so we take the No arrow to the else block: Cold.",
                "add_nodes": [],
                "add_arrows": [arrows["d2_no"]],
                "highlight_node": None,
                "blink_arrows": [arrows["d2_no"]],
            },
            {
                "text": "We go to the Cold box, then to the end.",
                "add_nodes": [nodes["cold"], nodes["end"]],
                "add_arrows": [arrows["cold_end"]],
                "highlight_node": nodes["cold"],
                "blink_arrows": [arrows["cold_end"]],
            },
        ]
        ...
        with self.voiceover(text="The other branches for Hot and Mild are not taken.") as tracker:
            for obj in [arrows["d1_yes"], nodes["hot"], arrows["hot_end"],
                        arrows["d2_yes"], nodes["mild"], arrows["mild_end"]]:
        ...
        line_voice_texts = [
            "We set temp to 3.",
            "First if checks if temp is greater than 30. It's false.",
            "This line is skipped.",
            "Then we check elif: temp > 15? Also false.",
            "This line is skipped.",
    ...
    def section_nested(self):
        ...
        steps = [
            ...
            {
                "text": "is_member is True, so we take the Yes arrow.",
                "add_nodes": [],
                "add_arrows": [arrows["d1_yes"]],
                "highlight_node": None,
                "blink_arrows": [arrows["d1_yes"]],
            },
            {
                "text": "Now we enter the inner if: check weight <= 5.",
                "add_nodes": [nodes["d2"]],
                "add_arrows": [],
                "highlight_node": nodes["d2"],
                "blink_arrows": [],
            ...
            {
                "text": "Weight is 4, so condition is true. We take the Yes arrow to Free shipping.",
                "add_nodes": [],
                "add_arrows": [arrows["d2_yes"]],
                "highlight_node": None,
                "blink_arrows": [arrows["d2_yes"]],
            },
            {
                "text": "We print Free shipping and go to end.",
                "add_nodes": [nodes["free"], nodes["end"]],
                "add_arrows": [arrows["free_end"]],
                "highlight_node": nodes["free"],
                "blink_arrows": [arrows["free_end"]],
            },
        ]
        ...
        with self.voiceover(text="The other paths (not member or weight > 5) are not taken.") as tracker:
            for obj in [arrows["d1_no"], nodes["standard"], arrows["standard_end"],
                        arrows["d2_no"], nodes["discount"], arrows["discount_end"]]:
                self.play(FadeIn(obj, scale=0.9), run_time=0.1)
                self.play(obj.animate.set_opacity(0.3), run_time=0.1)
        ...
        line_voice_texts = [
            "We set is_member to True.",
            "We set weight to 4.",
            "Outer if: is_member? True, so we enter.",
            "Inner if: weight <= 5? True, so we enter.",
            "We print 'Free shipping'.",
            "The else of the inner if is skipped.",
            "This line is skipped.",
            "The else of the outer if is skipped.",
            "This line is skipped."
        ]
        self.run_code_beat(
            desc, code, outputs,
    ...
    def run_code_beat(self, desc, code, outputs, exec_order, skip, filename, line_voice_texts):
        """
        Now accepts line_voice_texts (list of strings) for each line in exec_order.
        Each line will have its own voiceover block.
        """
        ...
        # Loop through each line in exec_order with voiceover
        for idx, line_idx in enumerate(exec_order):
            voice_text = line_voice_texts[idx] if idx < len(line_voice_texts) else ""
