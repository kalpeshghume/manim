...
class IfStatementsScene(VoiceoverScene):
    def construct(self):
        ...
        self.section_mobjects = []   # <-- CORRECT name (no underscore)
    ...
    def section_if(self):
        ...
        # We'll add group step by step, so we remove it from the scene
        self.remove(group)
        ...
        # Add the No arrow (not taken) dimmed
        ...
        # Track group and caption so they are managed by clear_section
        self.track(group)
        self.track(caption)

        self.wait(0.5)
        # Manually remove them now so code panel can appear; remove safely
