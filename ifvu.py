...
class IfStatementsScene(VoiceoverScene):
    ...
    def section_if(self):
        ...
        path_nodes = flowchart_data["path_nodes"]
        path_arrows = flowchart_data["path_arrows"]

        group.next_to(desc, DOWN, buff=0.35)
        self.add(group)  # add all at once with low opacity? We'll do stepwise.

        # We'll do stepwise: we'll not add all at once, but we'll add elements step by step in voiceover.
        # But we need to start with nothing.
        # So we'll remove group and add it stepwise.
        self.remove(group)

        # Define steps: each step has text, node(s) to add, arrow(s) to add, node to highlight, arrow to blink
        steps = [
            {
        ...
        # Also add the No arrow (not on path) but we can add it later dimmed.
        # We'll add it at the end with low opacity.
        ...
        # Now add the No arrow (not taken) with low opacity
